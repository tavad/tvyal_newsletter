...
transfers_total_chart <- 
  ...
  labs(
    ...
    subtitle = "Առաջին կիսամյակ, In a Year, Billions of USD",
...
transfers_cleant_reemap_data <- 
  ...
  mutate(
    country = ifelse(K_USD <= 1100, "Others", country)
  ) |> 
  group_by(country) |> 
...
transfers_clean |>
  ...
  labs(
    ...
    y = NULL,
    fill = NULL,
    title = "Ոչ կոմերցիոն մասնավոր փոխանցումներ / ՀՆԱ հարաբերություն"
  )
```
