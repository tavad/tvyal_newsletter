...
last_yer_growth_plot <- 
  ...
  scale_fill_manual(values = new_palette_colors[c(7,5,2,4,8,6)]) +
  labs(
    x = NULL,
    y = NULL,
  ) +
...
fertility_rate_plot_data |> 
  ...
  scale_color_manual(values = new_palette_colors[c(2,4,5,6,7,8)]) +
...
tibble(
  ...
  geom_text(
    data = tibble(x = 2, y = c(-0.7, -0.9), label = c("անկում", "աճ")),
    aes(x, y, label = label),
    color = new_palette_colors[1], hjust = 1,
  ) +
  scale_y_continuous(
...
plot_armenia_population_forecast <- 
  pop_forecast |> 
  filter(name == "Armenia") |> 
  mutate(
...
ggsave("plots/plot_sex_ratio.png", plot_sex_ratio, width = 9, height = 8)
