...
GDP_quarter_pct <- 
  GDP_quarter |> 
  filter(grepl("Domestic product", eng)) |> 
  transmute(date, GDP_growth = vol_YoY_pct/100 - 1) |> 
  na.omit()


GDP_quarter_contribution <- 
  GDP_quarter |> 
  left_join(GDP_quarter_pct, by = "date") |>
  group_by(eng) |> 
  mutate(
    vol_YoY_pct = vol_YoY_pct/100,
    contribution = lag(production) * (vol_YoY_pct - 1),
...
tbl_groupped <- 
  ...
  mutate(
    ...
    groupped_eng = case_when(
      code %in% c("D", "E") ~ "Energy and water",
      code %in% groupped_codes_ ~ paste(groupped_codes_, collapse = "+"),
      grepl("Domestic product", eng) ~ NA,
      TRUE ~ eng
    ),
  ...
  group_by(groupped_codes, date, groupped_eng) |> 
  summarise(
    contribution = sum(contribution),
    .groups = "drop"
  ) |> 
...
tbl_groupped |> 
  ...
  labs(
    ...
    captions = paste0(caption_eng, "    |    Data Source: armstat.am"),
    # title = paste("Factors Contributing to", year_, "Economic Growth")
    title = paste("Forecast of Factors that will Contribute to", "2024-I", "Economic Growth")
  ) +
  guides(
    linetype = guide_legend(override.aes = list(fill = "#FFFFFF00")),
    color = guide_legend(order = 2),
...
tbl_groupped <- 
  ...
  group_by(groupped_codes, date, groupped_arm) |> 
  summarise(
    contribution = sum(contribution),
    .groups = "drop"
  ) |> 
...
tbl_groupped |> 
  ...
  # geom_area() +
  geom_stream(type = "ridge", bw=0.9,  extra_span = 0.025,  n_grid = 1500)  +
  # scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
