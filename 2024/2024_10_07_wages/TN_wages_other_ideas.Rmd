...
wages_db_clean_bc |>
  filter(
     date == ymd("2022-12-31")
  # nace_2_code == "G" & date == ymd("2024-01-31")
  ) |> view()
  
  mutate(
    across(c(wages, wages_cumulative), ~ifelse(nace_2_code == "G" & date == ymd("2024-01-31") & type_eng == "Non-public", .x * 10, .x))
  )
...
wage_quantile |> 
  ...
  mutate(
    ...
    deciles_name_arm = case_when(
      deciles == "Q1" ~ "Q3 (ամենացածր 25% շեմ)",
