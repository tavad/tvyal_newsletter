...
wages_db_clean |> 
  mutate(
    across(c(wages, wages_cumulative), ~ifelse(nace_2_code == "G" & date == ymd(2024-01-31), .x * 10, .x))
  )
  view()
...
wages <- wages_db_clean_test$wages_cumulative
weights <- wages_db_clean_test$employees
...
inf_cumprod <- 
  ...
  filter(date <= max(wage_deciles$date)) |>
  select(-year) |> 
  arrange(desc(date)) |> 
  mutate(
    adj_price = cumprod(cpi_m),
...
wages_db_clean |> 
  filter(
    ...
    nace_2_code %in% LETTERS,
    type_eng != "Total",
    date %in% ymd("2022-12-31", "2024-01-31")
  ) |> view()
