...
GDP_plot <- 
  ...
  mutate(
    ...
    country = case_when(
      ...
      country == "Azerbaijan" ~ "Ադրբեջան",
      country == "Georgia" ~ "Վրաստան",
      TRUE ~ country
    ),
    # Translate facet titles
    name = case_when(
      name == "GDP (current US$, Billion)" ~ "ՀՆԱ (ընթացիկ ԱՄՆ $, միլիարդ)",
      name == "GDP, PPP (current international $, Billion)" ~ "ՀՆԱ, ԳՓՀ (ընթացիկ միջազգային $, միլիարդ)",
      name == "GDP per capita (current US$)" ~ "Մեկ շնչի հաշվով ՀՆԱ (ընթացիկ ԱՄՆ $)",
      name == "GDP per capita, PPP (current international $)" ~ "Մեկ շնչի հաշվով ՀՆԱ, ԳՓՀ (ընթացիկ միջազգային $)",
      TRUE ~ name
    ),
    name = fct_inorder(name),
  ) %>% 
  ...
  labs(
    ...
    y = NULL,
    color = NULL,
    title = "Հարավային Կովկասի երկրների ՀՆԱ-ի համեմատություն",
    subtitle = "1990-ից մինչև 2023 թվականները",
    caption = "Հեղինակ՝ Աղասի Թավադյան   |   tvyal.com   |   tavadyan.com   |   Տվյալների աղբյուր՝ Համաշխարհային բանկ"
  )

ggsave("~/Downloads/plots/coucase_gdp.png", ggplot2::last_plot(), width = 12, height = 8)
