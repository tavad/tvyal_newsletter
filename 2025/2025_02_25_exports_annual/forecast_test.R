...
save_folder <- "/home/tavad/docs/emma_diso/tables_and_plots"


trade_data_dir <- "/home/tavad/proj/2021_CBA_grant/armstat_data"
arm_trade_country <-  read_csv(file.path(trade_data_dir, "armstat_country.csv"))
arm_trade_commodity <- read_csv(file.path(trade_data_dir, "armstat_commodity.csv"))
...
last_date <- arm_trade_commodity_groupped$date |> max()
last_date_ext <- last_date + months(1) - days(1)
last_sum_value <- arm_trade_commodity_groupped |> 
  filter(direction == "export", date > max(date) - years(1)) |> 
  summarise(value = sum(value) / 1e6) |> 
  pull(value)
mean_value_71 <- arm_trade_commodity_groupped |> 
  filter(direction == "export", date > max(date) - months(3), hs2 == 71) |> 
...
arm_trade_71_add <- 
  ...
  bind_rows(
    tibble(
      date = seq.Date(last_date + months(1), last_date + years(2), "months"),
      value_71       = mean_value_71,
      ribbon_high_71 = 500000,
  ...
  mutate(
    across(matches("High|low"), ~ifelse(date < last_date, NA, .x))
  ) |> 
...
data_select2 |> 
  ...
  geom_segment(
    aes(y = 8.5, x = ymd("2020-01-01"), yend = 8.5, xend = last_date_ext),
    arrow = arrow(length = unit(0.02, "npc")),
    color = new_palette_colors[3],
    lineend = "round", linejoin = "mitre"
  ) +
  geom_segment(
    aes(y = 0, x = last_date_ext, yend = last_sum_value, xend = last_date_ext), 
  ...
  geom_text(
    aes(
      ...
      label = "2024թ. արտահանումը կազմել է 13.1 մլրդ դոլար,\nորից 61.4%-ը «Թանկարժեք քարեր և մետաղներ»-ի (ԱՏԳ ԱԱ 71) արտահանումն է\n\n2021թ. արտահանումը 3.0 մլրդ դոլար էր,\nայս ապրանքախմբի մասնաբաժինը՝ 10.8%"
    ),
    size = 3.2,
    color = "gray40",
  ...
  scale_color_manual(
    ...
    labels = c("Ամբողջական արտահանումը", "Արտահանումը, առանց (71) թանկարժեք քարեր և մետաղների")
  ) +
  ...
  labs(
    ...
    subtitle = "2025թ․ արտահանումը նվազելու է 30%-ով՝ կազմելով մոտ 9 մլրդ դոլար\nտվյալները 12-ամսյա կտրվածքով",
    caption = caption_f()
  )

ggsave(
  "~/Downloads/plots/exports_forecast.png", ggplot2::last_plot(),
  width = 12, height = 8
)
