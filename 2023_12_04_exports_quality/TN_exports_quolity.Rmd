...
partner_eports_year %>% 
  left_join(partners_dic, by = "partner_region") %>% 
  # filter(year <= year_select) %>% 
  ggplot(aes(year, exp_year_region/ 1e6,
             color = partner_region_arm,
             linetype = partner_region_arm
             )) +
  geom_line(size = 1.5) +
  ...
  labs(x = "",
       y = "մլն. ԱՄՆ դոլար",
       title = "Հայաստանի արտահանումը դեպի գործընկեր տարածաշրջաններ") +
  # labs(x = "",
  #      y = "Million USD",
  #      title = "Exports of Armenia to partner regions by year") +
