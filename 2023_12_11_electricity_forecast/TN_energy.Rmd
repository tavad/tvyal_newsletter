...
petrol_dict <- 
  tibble(
    ...
    density = c(0.737, 0.885, NA, 0.78) # kg/m3
  )

fuel_imports <- 
  read_csv("../2023_11_20_petrol_price/fuel_imports.csv") |> 
...
fuel_price_plot_3
plot_petrolium_imports_1 |>
  filter(commodity_code %in% c(271121)) |> 
  mutate(
    year = year(period),
    month = month(period)
  ) |> 
  ggplot(aes(month, netweight_kg / 1e6, color = year, group = year)) +
  geom_line(alpha = 0.8) +
  # scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
...
natural_gas <- 
  comtrade_data |> 
  filter(
    commodity_code %in% c(271121)
  )

natural_gas |> write_csv("natural_gas_trade_armenia.csv")

# Pipelines

natural_gas |> 
  filter(mot_code != 9110) |> 
  view()
...
petrol_imports_price <- 
  ...
  mutate(
    cubic_meter_price = trade_value_us / netweight_kg / density,
    # cubic_meter_price = ifelse(cubic_meter_price == Inf, NA, cubic_meter_price),
...
petrol_imports_price |> 
  ggplot(aes(period, cubic_meter_price, color = name_short_eng)) +
